...
screen_log 1 "Mode: ${RUN_MODE:-dev}"

# 1. Check Mount
if grep -q "/usr/share/blanket/screensaver" /proc/mounts; then
    screen_log 2 "Mount: OK"
else
    screen_log 2 "Mount: MISSING"
fi

# 2. Check Extension Scheduler
if ps auxww | grep "scheduler.sh" | grep -qv grep; then
    screen_log 3 "Scheduler: RUNNING"
else
    screen_log 3 "Scheduler: STOPPED"
fi

# 3. Check Log File
LOGFILE="/mnt/us/extensions/onlinescreensaver/logs/onlinescreensaver.txt"
if [ -f "$LOGFILE" ]; then
    screen_log 4 "Log: CREATED ($(du -h "$LOGFILE" | awk '{print $1}'))"
else
    screen_log 4 "Log: MISSING"
fi

# 4. Check Upstart Configs
if [ -f /etc/upstart/onlinescreensaver-mount.conf ]; then
    screen_log 5 "Upstart Job: INSTALLED"
else
    screen_log 5 "Upstart Job: MISSING"
fi

# 5. Check Permissions & Folder
if [ -d "/mnt/us/extensions/onlinescreensaver" ]; then
    screen_log 6 "Ext Folder: EXISTS"
    if [ -w "/mnt/us/extensions/onlinescreensaver/logs" ]; then
        screen_log 7 "Logs Folder: WRITABLE"
    else
        screen_log 7 "Logs Folder: NOT WRITABLE"
    fi
else
    screen_log 6 "Ext Folder: MISSING"
fi
...
if [ -d "/usr/share/blanket/screensaver" ]; then
    ...
    screen_log 8 "Mount Files: $SC_COUNT"
    if [ $SC_COUNT -eq 0 ]; then
        screen_log 9 "!! NO IMAGES IN MOUNT !!"
    fi
fi

screen_log 10 "Press any key/button to exit."
